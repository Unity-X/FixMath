...
public static class FixRandomExtensions
{
    ...
    /// </summary>
    /// <param name="plateauWidth">The width of the plateau, between 0 and 1. 0 resulting in a triangular distribution /\. 1 resulting in a flat distrubution —.</param>
    public static fix NextPlateauRatio(this ref FixRandom random, fix plateauWidth)
    {
        if (plateauWidth < 0 || plateauWidth > 1)
            throw new System.ArgumentOutOfRangeException($"{nameof(plateauWidth)} needs to be between 0 and 1");


        fix t = 1 - plateauWidth;
        fix pAreaProportion = plateauWidth / (plateauWidth + (t * fix.Half));

        bool onPlateau = random.NextFixRatio() < pAreaProportion;

        fix halfP = plateauWidth / 2;
        if (onPlateau)
        {
    ...
    /// Returns a random value between 0 and 1 with a plateau-shaped distribution. If you were to plot all the results, it would give a shape like this: /¯¯\
    /// </summary>
    /// <param name="plateauWidthRatio">The width of the plateau, between 0 and 1. 0 resulting in a triangular distribution /\. 1 resulting in a flat distrubution —.</param>
    public static fix NextPlateau(this ref FixRandom random, fix min, fix max, fix plateauWidthRatio)
    {
        return NextPlateauRatio(ref random, plateauWidthRatio) * (max - min) + min;
    }
}
